 ...
 */
class WebForthVM extends Thread
{   // Version VER.EXT
    // Keyboard Event buffer
    public static final int KEYBUFLEN = 256;
    public static final int MAX_CHAR = 511; //5 CMJ
    // Output character buffer
    public static final int OUTBUFLEN = 256;
    // Block I/O data
    public static final int BLKLEN = 1024;
    public static final int NUMBLOCKS = 100;
    protected static final int BUFS = 8;
    static final int VER = 1;
    static final int EXT = 0x00;
    // Memory is 64K words, I/O buffer is 8 BLOCKS of 1024 words
    static final int MEMORY_SIZE = 0x00010000 + 0x2000;
    static final int PRIMITIVE = 0x00020000;
    static final int TRUE = -1;
    static final int FALSE = 0;
    static final int BKSPP = 8;    // backspace
    static final int LF = 10;   // line feed
    static final int CRR = 13;   // carriage return
    static final int ERR = 27;   // Escape character
    ...
    static final int CELLL = 1;
    static final int BASEE = 10;
    static final int VOCS = 8;
    static final int EM = 0x0000FF00;// top of memory
    static final int BUFFER0 = 0x00010000;// I/O buffer addr
    static final int COLDD = 0x0100;       // cold start vector
    static final int US = 128 * CELLL;    //  user area size in cells
    static final int RTS = 128 * CELLL;    // return stack/TIB size
    static final int UP = 0;            // start of user variables (US)
    static final int RPP = EM - 8 * CELLL;   // start of return stack (RP0)
    static final int TIBB = RPP - RTS;      // terminal input buffer (TIB)
    static final int SPP = TIBB - 8 * CELLL; // start of data stack (SP0)
    static final int BSPP = EM - 256 * CELLL; // bottom of data stack
    static final int NAMEE = BSPP - 8 * CELLL; // name dictionary
    static final int CODEE = COLDD + US;     // code dictionary
    static final int TIB_SIZE = 80;       // size of terminal input buffer //1 PSHP-5
    static final int PAD_OFFSET = 80;       // offset of PAD from HERE       //1 PSHP-5
    static final int PAD_SIZE = 84;       // size of PAD                   //1 PSHP-5
    // Tracing mode constants
    static final int NOTRACE = 0;
    static final int STEPPING = 2;
...
//    static final int FIBLENGTH = 129;  // one more than max line length in source file //4 PSHP-2
    ...
    static final int FIBLENGTH = 256;  // one more than max line length in source file //6 CJ
    static final int RSLASHO = 1;      // file access method (fam) value for read only //5 PSHP
    // Lexicon constants
    static final int COMPO = 0x00000040;  // lexicon compile-only bit
    static final int IMEDD = 0x00000080;  // lexicon immediate bit
    static final int MASKK = 0xFFFFFF3F;  // lexicon bit mask
    // User Variables, allocated in m[]
    static final int SZERO = 1 + UP;     // SP0     Points to bottom of data stack
    static final int RZERO = 2 + UP;  // RP0        Points to bottom of return stack
    static final int TQKEY = 6 + UP;  // '?KEY  Execution vector of ?KEY
    static final int TEMIT = 7 + UP;  // 'EMIT  Execution vector of EMIT
    static final int TEXPE = 8 + UP;  // 'EXPECT    Execution vector of EXPECT
    static final int TTAP = 9 + UP;  // 'TAP   Execution vector of TAP
    static final int TECHO = 10 + UP; // 'ECHO  Execution vector of ECHO
    static final int TPROM = 11 + UP; // 'PROMPT Execution vector of PROMPT
    static final int BASE = 12 + UP; // BASE   Radix base for numeric I/O
    static final int TEMP = 13 + UP; // tmp        Temp. storage for parse and find
    static final int SPAN = 14 + UP; // SPAN   Character count for EXPECT
    static final int INN = 15 + UP; // >IN        Pointer for parsing input stream
    static final int NTIB = 16 + UP; // #TIB   Current count and address of the
    static final int CSP = 18 + UP; // CSP        Stack pointer for error checking
    static final int TEVAL = 19 + UP; // 'EVAL  Execution vector of EVAL
    static final int TNUMB = 20 + UP; // 'NUMBER    Execution vector of NUMBER?
    static final int HLD = 21 + UP; // HLD        Pointer in numeric output string
    static final int HANDL = 22 + UP; // HANDLER    Ret stack ptr for error handling
    static final int CNTXT = 24 + UP; // CONTEXT    Area to specify vocabulary search
    static final int CRRNT = 32 + UP; // CURRENT    Points to vocabulary to extend
    static final int CP = 34 + UP; // CP     Top of the code dictionary
    static final int NP = 35 + UP; // NP     Top of the name dictionary
    static final int LAST = 36 + UP; // LAST   Last name in the name dictionary
    static final int BLK = 37 + UP; // BLK        BLOCK # being interpreted
    static final int BUF0 = 38 + UP; // BUF0   BLOCK # for buffer area 0
    // = 45             BLOCK # for buffer area 7
    static final int UPD0 = BUF0 + BUFS;
    // = 46             Flag that buffer 0 is updated
    // = 54             Flag that buffer 7 is updated
    static final int LSTBUF = UPD0 + BUFS;
    //          BUF # most-recently used
    static final int BLOCKS = LSTBUF + 1;
    //          Number of blocks in current BLOCK file
    static final int CURBLK = BLOCKS + 1;
    //          BLOCK # of most recently accessed BLOCK
    static final int CURPOS = CURBLK + 1;
    //          Current address inside current block
    static final int TRACING = CURPOS + 1;
    //          Indicates tracing mode
    static final int NULLSTR = TRACING + 1;
    //          Empty error string
    static final int CAPS = NULLSTR + 1;
    //          Indicates case sensitivity, TRUE for case-sensitive
    static final int FG_CLR = CAPS + 1;
    //          Current forground color for text output
    static final int BG_CLR = FG_CLR + 1;
    //          Current background color for text output
    static final int STATE = BG_CLR + 1;
    //          Interpretation/compilation state
    static final int LEAVES = STATE + 1;
    //          Pointer to unresolved LEAVEs
    static final int DEF_BG_CLR = LEAVES + 1;
    //          Default background color
    static final int FORTH_CLR = DEF_BG_CLR + 1;
    //         terminal input buffer.
    //          Color for default FORTH output
    static final int CURSOR_CLR = FORTH_CLR + 1;
    //          Color for cursor
    static final int ERROR_CLR = CURSOR_CLR + 1;
    //          Color for FORTH error text output
    static final int USER_CLR = ERROR_CLR + 1;
    //          Color for echoing user keystrokes
    static final int ABORT$ = USER_CLR + 1;   //1 PSHP
    //          Address of ABORT" string
    static final int NAME$ = ABORT$ + 1;     //1 PSHP
    //          Address of unrecognized name string
    static final int DEEPBLK = NAME$ + 1;    //1 PSHP
    //         order (8 vocabularies)
    //          Holds deepest BLK  # when an exception is thrown
    static final int DEEPPOS = DEEPBLK + 1;  //1 PSHP
    //   33;            Vocabulary link pointer
    //          Holds >IN value associated with DEEPBLK
    static final int SID = DEEPPOS + 1;                                              //4 PSHP-2
    //          Holds SOURCE-ID value                   //4 PSHP-2
    static final int DEEPSID = SID + 1;                                              //4 PSHP-2
    //          Holds deepest SOURCE-ID when an exception is thrown //4 PSHP-2
    static final int DEEPLINE = DEEPSID + 1;                                         //4 PSHP-F
    // Holds line number of deepest file when an exception is thrown //4 PSHP-F
    static final int ROWS = DEEPLINE + 1;                                            //5 CMJ
    //          Holds height of console in characters   //5 CMJ
    static final int COLS = ROWS + 1;                                                //5 CMJ
    // Primitive word Symbols
    static final int BYE = 1;  // BYE
    static final int QRX = 2;  // ?RX
    static final int TXSTO = 3;  // TX!
    static final int STOIO = 4;  // !IO
    static final int DOLIT = 5;  // doLIT
    static final int EXIT = 6;  // EXIT
    static final int EXECU = 7;  // EXECUTE
    static final int DONXT = 8;  // next
    static final int QBRAN = 9;  // ?branch
    static final int BRAN = 10; // branch
    static final int STORE = 11; // !
    static final int AT = 12; // @
    static final int CSTOR = 13; // C!
    static final int CAT = 14; // C@
    static final int RFROM = 15; // R>
    static final int RAT = 16; // R@
    static final int RPAT = 17; // RP@
    static final int RPSTO = 18; // RP!
    static final int TOR = 19; // >R
    static final int SPAT = 20; // SP@
    static final int SPSTO = 21; // SP!
    static final int DROP = 22; // DROP
    static final int DUPP = 23; // DUP
    static final int SWAP = 24; // SWAP
    static final int OVER = 25; // OVER
    static final int ROT = 26; // ROT
    //          Holds width of console in characters    //5 CMJ

// .........................................................
    static final int ZLESS = 27; // 0<
    static final int ANDD = 28; // AND
    static final int ORR = 29; // OR
    static final int XORR = 30; // XOR
    static final int UPLUS = 31; // UM+
    static final int UMMOD = 32; // UM/MOD
    static final int UMSTA = 33; // UM*
    static final int INC = 34; // 1+
    static final int DEC = 35; // 1-
    static final int DEPTH = 36; // DEPTH
    static final int PICK = 37; // PICK
    static final int FIND = 38; // FIND
    static final int UINIT = 39; // UINIT
    static final int VERBO = 40; // VERBOSE
    static final int QUIET = 41; // QUIET
    static final int BLOCKRD = 42; // block@
    static final int BLOCKWR = 43; // block!
    static final int SCRNREF = 44; // screen-refresh
    static final int PLOOP = 45; // +LOOP
    static final int EXECTR = 46; // EXEC-TRACE
    static final int ATXY = 47; // AT-XY
    static final int PAGE = 48;  // PAGE
    static final int MS = 49;  // MS (milliseconds)
    static final int PSEMI = 50; // (;)       //1 PSHP
    static final int doCON = 51; // doCON     //1 PSHP
    static final int doUSER = 52; // doUSER    //1 PSHP
    static final int ZEQUAL = 53; // 0=        //1 PSHP-5
    static final int OPENFILE = 54; // OPEN-FILE                             //5 PSHP
    static final int CLOSEFILE = 55; // CLOSE-FILE                            //5 PSHP
    static final int FILEPOSITION = 56; // FILE-POSITION                         //5 PSHP
    static final int FILESIZE = 57; // FILE-SIZE                             //5 PSHP
    static final int READFILE = 58; // READ-FILE                             //5 PSHP
    static final int READLINE = 59; // READ-LINE                             //5 PSHP
    static final int REPOSITIONFILE = 60; // REPOSITION-FILE                       //5 PSHP
    static final int FILENAME = 61; // FILE-NAME                             //5 PSHP
    static final int LINENUMBER = 62; // LINE-NUMBER                           //5 PSHP
    static final int SETLINENUMBER = 63; // SET-LINE-NUMBER                       //5 PSHP
    static final int REPEATLINE = 64; // REPEAT-LINE                           //4 PSHP-2
    static final int KEYQ = 65; // KEY?                                  //4 CMJ
    static final int GETXY = 66; // GET-XY                                //5 CMJ
    static final int NAMESEARCH = 67; // NAME-SEARCH                           //5 CMJ
    // All primitives should have lower numbers than this one!!
    static final int __the_last__ = 68;                                          //5CMJ
    // Primitive word Symbols
    static final String primName[] =
            {
                    "( ??? )", "BYE", "?RX", "TX!",
    ...
    public char[] keybuf;   // Circular buffer
    public int keyp;     // "Put" buffer location
    public int keyg;     // "Get" buffer location
    public char[] outbuf;   // Circular buffer
    public int outp;     // "Put" buffer location
    public int outg;     // "Get" buffer location
    protected int[] m;          // Accessible eForth memory
    protected int ip;         // instruction pointer
    protected int sp;         // parameter stack pointer
    protected int rp;         // return stack pointer
    protected boolean bGone;    // Indicates time to leave
    protected int blocks;
    protected char blkfile[];  // simulated file
    // Kernel markers
    protected int _LINK = 0;        // force a null link
    protected int _NAME = NAMEE;    // initialize name pointer
    protected int _CODE = CODEE;    // initialize code pointer
    protected int _USER = 4 * CELLL;  // first user variable offset
    // Reference to Applet
    NoWebForth app;
    Hashtable files = new Hashtable(); // holds references to open files               //5 PSHP
    // File input data                                                                 //5 PSHP
    private int nextFileid = 1;        // used to track assignment of fileids          //5 PSHP
    public WebForthVM (NoWebForth noWebForthApp) throws IOException
    {
        app = noWebForthApp;
        m = new int[MEMORY_SIZE];
    ...
    public synchronized char dequeueKey ()
    {
...
//4 CMJ        for (i = 0 ; i < 64 ; i++)  // Clear the line
    ...
    void UInit ()
    {
        ...
        m[CP] = CTOP;
        m[NP] = _NAME;
        m[LAST] = _NAME + (2 * CELLL);
        m[BLK] = 0;
        for (i = 0; i < BUFS; i++)
        {
            m[BUF0 + i] = -1;   // buffer unused
            m[UPD0 + i] = 0;    // buffer not updated
        }
        m[LSTBUF] = BUFS - 1;
        m[BLOCKS] = blkfile.length / BLKLEN;
        m[CURBLK] = -1;
        m[CURPOS] = 0;
        m[TRACING] = NOTRACE;
        m[NULLSTR] = 0;
        m[CAPS] = 0;  // not case-sensitive by default
        m[DEF_BG_CLR] = app.paramDEF_BG_CLR;
        m[FORTH_CLR] = app.paramFORTH_CLR;
        m[CURSOR_CLR] = app.paramCURSOR_CLR;
        m[ERROR_CLR] = app.paramERROR_CLR;
        m[USER_CLR] = app.paramUSER_CLR;
        m[FG_CLR] = m[FORTH_CLR];   //3 CMJ Initial values
        m[BG_CLR] = m[DEF_BG_CLR];  //3 CMJ Initial values
        m[ROWS] = app.paramROWS;  //5 CMJ
        m[COLS] = app.paramCOLS;  //5 CMJ
    }

    //Initialize Vectored Routines
    void VInit () throws IOException
    {
        m[TQKEY] = doFind("?RX");
        m[TEMIT] = doFind("TX!");
        m[TEXPE] = doFind("accept");
        m[TTAP] = doFind("kTAP");
        m[TECHO] = doFind("TX!");
        m[TPROM] = doFind(".OK");
        m[TEVAL] = doFind("$INTERPRET");
        m[TNUMB] = doFind("NUMBER?");
    }
    ...
    protected void header (String name)
    {
        ...
        m[na++] = here();       // set token pointer
        m[na++] = _LINK;        // set link to previous name
        _LINK = na;             // next link points to this name string
        m[na++] = _LEN;
        for (int i = 0; i < _LEN; i++)
        {                       // lay down letters of name
            m[na++] = (int) (name.charAt(i));
        }
        m[LAST] = _LINK;        // Update LAST user variable
    }

    // Build entry for primitive routine in dictionary
    protected void primitive (String name, int inst)
    {
        header(name);                   // Build header
        m[_NAME] = inst + PRIMITIVE;    // Set CA to token
    }

    //////////////////////////////////////////////////////////
    // Java to Forth "Meta" compiler
    // These routines allow this Java class to build an
    // ANS Forth in the memory array m[]
    //////////////////////////////////////////////////////////
    ...
    protected void literal (int lit)
    {
        code(DOLIT);
        comma(lit);
    }
    ...
    protected void strlit (String s)
    {
        m[here()] = s.length();
        m[CP]++;                        // Increment "HERE"
        for (int i = 0; i < s.length(); i++)
        {
            m[here() + i] = (int) (s.charAt(i));
        }
        m[CP] += s.length();            // Update HERE
    }
    ...
    protected void allot (int n)
    {                                                  //5 PSHP
        m[CP] += n;                                                                //5 PSHP
    }                                                                              //5 PSHP

    // Determines if name in dictionary matches a given string
    protected boolean matched (String str, int s)
    {
    ...
    protected int doFind (String name, int na)
    {
        int ca = -1;
        int w = na;
        boolean found = false;
    ...
    protected int doFind (String name) throws IOException
    {
        int na = _LINK;
    ...
    String addrLenToString (int addr, int len)
    {                                    //5 PSHP
        StringBuffer sb = new StringBuffer(len);                                   //5 PSHP
        for (int i = 0; i < len; i++)
        {                                            //5 PSHP
            sb.append((char) m[addr + i]);                                         //5 PSHP
        }                                                                          //5 PSHP
    ...
    protected void call (String name) throws IOException
    {
        int ca = doFind(name);
        if (ca < 0)
        {
    ...
    protected void look () throws IOException
    {
        int start = pop();
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            printName(m[start + i]);
            app.print("\t");
            app.print(String.valueOf(m[start + i]));
            app.print("\t");
            app.print(String.valueOf((char) m[start + i]));
            app.print("\n");
        }
    }
    ...
    protected int here ()    // Opening for new code or data
    {
        return m[CP];
    }

    protected void printName (int inst)
    {
        String name;
    ...
    protected String getName (int ca)
    {
        int w = m[LAST];
        boolean found = false;
        String name = "( ??? )";
    ...
    protected int codeField (int nameAddr)
    {
        return nameAddr - 2;
    }

    // makes a Java.lang.String from a counted Forth string                        //5 PSHP
    protected String makeString (int n)                                             //5 PSHP
    {                                                                              //5 PSHP
        StringBuffer sb = new StringBuffer();                                      //5 PSHP
        for (int i = 0; i < m[n]; i++)                                           //5 PSHP
        {
            sb.append((char) m[n + 1 + i]);                                         //5 PSHP
        }
        String str = new String(sb);                                               //5 PSHP
        return str;                                                                //5 PSHP
    }                                                                              //5 PSHP

    protected int linkField (int nameAddr)
    {
        return nameAddr - 1;
    }
    ...
    public void inform (int inst) throws IOException
    {
        ...
        if ((SPP - sp) > 0)
        {
            ...
            for (int i = 1; i < times; i++)
            {
                ...
                {
                    app.print("\t");
                    app.print(String.valueOf(m[rp + i]));
                    app.print("\t\t\t\t");
                }
                else
                {
    ...
    protected void compBEGIN ()
    {
        mark();
    }

    protected void compUNTIL ()
    {
        code(QBRAN);
        comma(pop());  // lay down address of loop top
    }

    protected void compAGAIN ()
    {
        code(BRAN);
        comma(pop());  // lay down address of loop top
    }

    protected void compWHILE ()
    {
        compIF();
        swap();
    }

    protected void compREPEAT ()
    {
        compAGAIN();
        m[pop()] = here();
    }
    ...
    protected void compTHEN ()
    {
        m[pop()] = here();  // Fix up a branch target
    }

    protected void compELSE ()
    {
        code(BRAN);    // lay down an unconditional branch
        push(here());  // remember place to put branch target
        comma(0);      // temporary branch target, fixed later
        swap();        // get previous unresolved branch
        m[pop()] = here(); // Fix it to branch here (the else clause)
    }
...
//4 CMJ                    ch = (char)(pop() & MAX_CHAR);
            ...
            case DONXT:
                i = m[rp];      // i is loop index
                rp++;
                if (i > 0)
                {
                    i--;        // decrement index
                    rp--;
                    m[rp] = i;  // put back on stack
                    ip = m[ip]; // go back to begining of loop
                }
                else
                {
                    ip++;
            ...
            case QBRAN:
                if (pop() == FALSE)
                {
                    ip = m[ip];     // reset ip to inline address
                }
                else
                {
            ...
            case STORE:
                a = pop();      // a is address
                m[a] = pop();   // store number from stack in memory
                break;          // at address a

            // @        ( a -- w )  Push memory data onto data stack.
            case AT:
                a = pop();      // a is address
                push(m[a]);     // fetch data at address a in memory
...
//                    if (i < m[rp])      // is index less than limit?
                boolean f1 = ((m[rp] - i) > 0);
                ...
                {
                    ip = m[ip];     // Yes: reset ip to branch
                    // back to inline address
                    rp--;
                    m[rp] = i;  // save new index
                }
                else
                {
                    ip++;           // bump ip past branch addr
...
//1 PSHP new primitives
            ...
            case ZEQUAL:                                           //1 PSHP-5
                m[sp + 1] = (m[sp + 1] == 0) ? TRUE : FALSE;       //1 PSHP-5
                break;                                             //1 PSHP-5
...
// contains jumps which are too long or some other reason. CMJ 18-7-99
    ...
    public void doMorePrim (int inst) throws IOException
    {
        int a, b, c, i, n;     // temporary integers
        char ch;                // temporary character
        WebForthFile file;      // temporary file reference                        //5 PSHP
...
//4 PSHP-2                        push(0); push(-38);                                        //4 PSHP
//4 PSHP-2                    } else {                                                       //4 PSHP
//4 PSHP-2                        push(file.readFile(m, c, b)); push(0);                     //4 PSHP
//4 PSHP-2                    }                                                              //4 PSHP
//4 PSHP-2                    break;                                                         //4 PSHP
//4 PSHP-2                                                                                   //4 PSHP
...
//4 PSHP-2                        } else {                                                   //4 PSHP
//4 PSHP-2                            push(file.readLine(m, c, b)); push(TRUE); push(0);     //4 PSHP
//4 PSHP-2                        }                                                          //4 PSHP
//4 PSHP-2                    }                                                              //4 PSHP
//4 PSHP-2                    break;                                                         //4 PSHP
//4 PSHP-2                                                                                   //4 PSHP
...
//4 PSHP-2                        push(0); push(-38);                                        //4 PSHP
//4 PSHP-2                    } else {                                                       //4 PSHP
//4 PSHP-2                        push(file.readName(m, c, b)); push(0);                     //4 PSHP
//4 PSHP-2                    }                                                              //4 PSHP
//4 PSHP-2                    break;                                                         //4 PSHP
//4 PSHP-2                                                                                   //4 PSHP
...
//                                new URL(app.getCodeBase(), fileName).openStream());  //4 PSHP-2
                        ...
                        files.put(new Integer(n), file);                       //4 PSHP-2
                        push(n);
                        push(0);                                      //4 PSHP-2
                    }
...
//                    }
            ...
            case CLOSEFILE:                                                    //4 PSHP
                a = pop();                                                     //4 PSHP
                push((files.remove(new Integer(a)) == null) ? -38 : 0);       //4 PSHP
                break;                                                         //4 PSHP
            //4 PSHP
            ...
            case FILEPOSITION:                                                 //4 PSHP
                a = pop();                                                     //4 PSHP
                file = (WebForthFile) files.get(new Integer(a));                //4 PSHP
                if (file == null)
                {                                            //4 PSHP
                    push(0);
                    push(0);
            ...
            case REPEATLINE:                                                   //4 PSHP-2
                a = pop();                                                     //4 PSHP-2
                b = pop();                                                     //4 PSHP-2
                file = (WebForthFile) files.get(new Integer(a));                //4 PSHP-2
                if (file == null)
                {                                            //4 PSHP-2
                    push(0);
                    push(-38);                                        //4 PSHP-2
            ...
            case NAMESEARCH:                                                   //4 PSHP-3
                ...
                {                                            //4 PSHP-3
                    ...
                    for (i = 0; i < b; i++)
                    {                                  //4 PSHP-3
                        name[i] = Character.toUpperCase((char) (m[c + i]));     //4 PSHP-3
                    }                                                          //4 PSHP-3
                    ...
                    {                                     //4 PSHP-3
                        ...
                        {                         //4 PSHP-3
                            for (i = 0; i < b; i++)
                            {                          //4 PSHP-3
                                if (Character.toUpperCase((char) (m[a + 2 + i])) != //4 PSHP-3
                                        name[i])                                   //4 PSHP-3
                                {
                            ...
                            n = a + 1;                                         //4 PSHP-3
                            break caseInsensitiveSearch;                       //4 PSHP-3
                        }                                                      //4 PSHP-3
                    }                                                          //4 PSHP-3
                }
                else
                {                                                       //4 PSHP-3
                    ...
                    {                                     //4 PSHP-3
                        ...
                        {                         //4 PSHP-3
                            for (i = 0; i < b; i++)
                            {                          //4 PSHP-3
                                if (m[a + 2 + i] != m[c + i])                  //4 PSHP-3
                                {
                                    continue caseSensitiveSearch;              //4 PSHP-3
                                }
                            }                                                  //4 PSHP-3
                            n = a + 1;                                         //4 PSHP-3
                            break caseSensitiveSearch;                         //4 PSHP-3
                        }                                                      //4 PSHP-3
                    }                                                          //4 PSHP-3
                }                                                              //4 PSHP-3
                if (n != 0)                                                    //4 PSHP-3
...
//4 CMJ        header("MAX-CHAR");           code(doCON); comma(MAX_CHAR);
        ...
        m[firstEQK - 1] = 0;   // Disconnect the EQK word list from the Forth word list.
...
//1 PSHP                                compTHEN();
        ...
        call(".\"|");
        strlit("NoWebForth (Alpha) Version ");
        call("BASE");
        code(AT);    // save radix
        call("HEX");
        call("VER");
